...
src/server/routers/_app.ts   — Root router (AppRouter type exported from here)
       ...
       └── user              — userRouter (user.requestDungeonMaster, user.updateUsername, user.updatePassword)
       ...
       └── character         — characterRouter (character.create, character.update, character.list, character.listActive, character.listRetired, character.retire, character.activate, character.getById, character.updateHp, character.levelUp, character.updateSpellSlots, character.updateSubclass, character.longRest, character.shortRest, character.updateSkillProficiencies, character.updateSkillExpertise, character.updatePreparedSpells, character.updateFeatureUses, character.updateAbilityScores, character.updateActiveConditions, character.updateActiveDiseases, character.updateFeats, character.updateLanguages, character.updateLevelUpSelections, character.updateNotes, character.export, character.import)
...
| `character.update`                   | mutation | Update character identity and combat fields (name, race, class, alignment, background, backstory, languages, ability scores, maxHp, armorClass, speed, skillProficiencies, skillExpertise); clamps currentHp if maxHp is reduced; does not allow editing level, currentHp, or tempHp |
| `character.list`                     | query    | List characters belonging to the authenticated user; accepts optional `{ isRetired: boolean }` filter to return only active or retired characters; includes `adventurePlayers` data (filtered to PENDING/ACCEPTED, take 1) with nested `adventure` (`id`, `name`, `source`) to indicate each character's active adventure status |
| `character.listActive`               | query    | Paginated list of active (non-retired) characters for the authenticated user; input `{ page, pageSize }` with defaults 1 and 20; ordered by `createdAt` desc; returns `{ characters, total, page, pageSize }` with `adventurePlayers` data |
| `character.listRetired`              | query    | Paginated list of retired characters for the authenticated user; input `{ page, pageSize }` with defaults 1 and 20; ordered by `retiredAt` desc (most recently retired first); returns `{ characters, total, page, pageSize }` with `adventurePlayers` data |
| `character.retire`                   | mutation | Retire a character; sets `isRetired: true` and `retiredAt` to current timestamp; rejects any ACCEPTED `AdventurePlayer` records for the character (sets status to REJECTED); validates character belongs to the user |
| `character.activate`                 | mutation | Reactivate a retired character; sets `isRetired: false` but preserves `retiredAt` for "recently retired" display; validates character belongs to the user |
...
| `character.updateLanguages`          | mutation | Persist the full list of language names as a JSON string[]; used during level-up when class/subclass features grant new languages                                                                                                                                                                                       |
| `character.updateLevelUpSelections`  | mutation | Persist a Record<string, string[]> of miscellaneous level-up selections (fighting styles, tool proficiencies, metamagic, invocations, pact boons, totem animals, etc.)                                                                                                                                                 |
| `character.updateNotes`              | mutation | Persist the character's free-text notes string                                                                                                                                                                                                                                                                          |
| `character.export`                   | query    | Export a character as a clean JSON object suitable for re-import; strips internal IDs (`id`, `userId`), relationships (`adventurePlayers`, `dmNotes`), and timestamps (`createdAt`, `updatedAt`); includes `_version` (1) and `_exportedAt` ISO timestamp for future compatibility; for spellcaster classes, includes a computed `spellcasting` object with `ability` (WIS/CHA/INT), `spellSaveDC`, and `spellAttackBonus` derived from the character's class and ability scores |
| `character.import`                   | mutation | Import a character from an exported JSON object, creating a new character for the authenticated user; accepts optional `name` override; validates all fields with Zod; sanitizes `currentHp` to not exceed `maxHp`; level clamped 1-20; ability scores clamped 1-30; ignores `_version` and `_exportedAt` metadata fields |
...
| `adventure.joinByCode`        | mutation | User requests to join an adventure via invite code and selected character; validates character ownership; rejects if the character is already in another adventure (PENDING or ACCEPTED status); if a REJECTED AdventurePlayer record exists for the same adventure+user, updates it to PENDING with the new characterId and resets `joinedAt`/`resolvedAt`; otherwise creates a new PENDING AdventurePlayer record with characterId |
...
- `src/lib/spellSlotData.ts` — spell slot tables for full/half/artificer/warlock casters; exports `getSpellSlots(className, level)` returning a 9-element array of total slots per spell level, `isSpellcaster(className)` returning true for classes with any spell slots, `isWarlock(className)` for Pact Magic handling, and `SPELLCASTING_TYPE: Record<string, SpellcastingType>` mapping class names to their casting type. Also re-exports from `spellcastingProgressionData.ts`.
- `src/lib/spellcastingProgressionData.ts` — extracts spellcasting progression data (cantrips known, spells known/prepared limits) from the class JSON files for both PHB and XPHB rules sources; exports `getCantripsKnown(className, level, rulesSource?)`, `getSpellsKnownOrPrepared(className, level, abilityMod, rulesSource?)`, `getSpellManagementType(className)` returning `"known"` | `"prepared"` | `null`, and `getWizardSpellbookSize(level, rulesSource?)`.
...
- `src/lib/levelUpChoicesData.ts` — defines `LEVEL_UP_CHOICES: LevelUpChoiceEntry[]` mapping class/subclass/level/rulesSource to arrays of choices (languages, skills, fighting styles, tools, cantrips, etc.); exports `getLevelUpChoices(className, level, rulesSource, subclass?)` returning all applicable `LevelUpChoice[]` for a given level-up. Covers: Drakewarden language grant, Ranger Favored Enemy languages, Fighter/Ranger/Paladin fighting styles, Sorcerer metamagic, Warlock invocations/pact boon, Bard College of Lore skills, Cleric Knowledge Domain languages+skills, Barbarian totem/primal knowledge, Rogue Arcane Trickster languages+tools, Monk subclass choices, Artificer tool proficiencies.
