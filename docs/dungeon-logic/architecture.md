...
- `src/lib/raceData.ts` — static race/species data for races from multiple D&D 5e sourcebooks: 9 PHB (2014), 10 XPHB (2024, includes Goliath, Orc, Aasimar), 14 VGM (Volo's Guide), 31 MPMM (Monsters of the Multiverse, including Genasi variants, Deep Gnome, Duergar, Eladrin, Fairy, Githyanki, Githzerai, Harengon, Minotaur, Sea Elf, Shadar-kai, Tortle, Yuan-ti), 4 ERLW (Eberron), 1 EGW (Wildemount), 3 VRGR (Van Richten's Guide to Ravenloft: Dhampir, Hexblood, Reborn), 5 GGR (Guildmasters' Guide to Ravnica: Centaur, Loxodon, Minotaur, Simic Hybrid, Vedalken), 2 MOT (Mythic Odysseys of Theros: Leonin, Satyr), 1 AI (Acquisitions Incorporated: Verdan), 6 AAG (Astral Adventurer's Guide: Astral Elf, Autognome, Giff, Hadozee, Plasmoid, Thri-kreen), 1 SCC (Strixhaven: Owlin) — 87 total entries. The `RaceInfo.source` field is typed as `string` to support all sourcebook codes (PHB, XPHB, VGM, MPMM, ERLW, EGW, VRGR, GGR, MOT, AI, AAG, SCC). Exports `AbilityScoreBonus` interface (ability name, amount, and optional `"choice"` type for races like Half-Elf and MPMM flexible ASIs), `RacialTrait` and `RaceInfo` interfaces (includes `abilityBonuses: AbilityScoreBonus[]` with structured ASI data per race), `RACES: RaceInfo[]` (all versions), `RACE_SOURCES: string[]` (sorted unique source codes), `getRaceByNameAndSource(name, source)` for source-aware lookup, and `getRaceByName(name)` which returns the PHB version for backward compatibility.
