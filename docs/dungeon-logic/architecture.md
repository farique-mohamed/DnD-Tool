...
| `character.export`                   | query    | Export a character as a clean JSON object suitable for re-import; strips internal IDs (`id`, `userId`), relationships (`adventurePlayers`, `dmNotes`), and timestamps (`createdAt`, `updatedAt`); includes `_version` (1) and `_exportedAt` ISO timestamp for future compatibility |
